...
class Scene82DeferredShadingVolumes private constructor(
    ...
    override fun init(width: Int, height: Int) {
        ...
        lights.keys.forEachIndexed { i, position ->
            ...
            val color = lights.getValue(position)
            lightingProgram.setFloat3("lights[$i].Color", lights.getValue(position))
            val constant = 1.0f
            val linear = 0.7f
            val quadratic = 1.8f
            lightingProgram.setFloat("lights[$i].Linear", linear)
            lightingProgram.setFloat("lights[$i].Quadratic", quadratic)
            val maxBrightness = max(max(color.r, color.g), color.b)
            val radius = (-linear + sqrt(
                linear * linear - 4.0f * quadratic *
                        (constant - (256.0f / 5.0f) * maxBrightness)
            )) / (2.0f * quadratic)
            lightingProgram.setFloat("lights[$i].Radius", radius)
        }
    ...
    companion object {
        ...
        fun create(context: Context): Scene {
            ...
            return Scene82DeferredShadingVolumes(
                ...
                lightingFragmentShaderCode = resources.readRawTextFile(R.raw.advancedlighting_scene82_deferred_shading_volumes_lighting_frag),
