...
class TextRenderer(private val program: Program, private val fontTexture: Texture) {
    ...
    fun measure(text: String, scale: Float): Float2 {
        val measurement = Float2()
        var maxHeight = 0.0f
        text.forEach {
            val c = characters.getValue(it)
            measurement.x += c.advance * scale
            maxHeight = max(maxHeight, c.offset.y * scale + c.size.y * scale)
        }
    ...
    fun render(
        ...
        text.forEach {
            ...
            val y = position.y + c.offset.y * scale
            val width = c.size.x * scale
            val height = c.size.y * scale

            val vertices = floatArrayOf(
                x, y + height, c.texCoordsLowerLeft.x, c.texCoordsUpperLeft.y,
                x, y, c.texCoordsUpperLeft.x, c.texCoordsLowerLeft.y,
                x + width, y, c.texCoordsLowerRight.x, c.texCoordsLowerRight.y,
                x, y + height, c.texCoordsLowerLeft.x, c.texCoordsUpperLeft.y,
                x + width, y, c.texCoordsLowerRight.x, c.texCoordsLowerRight.y,
                x + width, y + height, c.texCoordsUpperRight.x, c.texCoordsUpperRight.y,
            )
            ...
            glBufferSubData(
                GL_ARRAY_BUFFER,
                0,
                Float.SIZE_BYTES * buffer.capacity(),
                buffer
            )
            glBindBuffer(GL_ARRAY_BUFFER, 0)
